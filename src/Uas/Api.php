...
final class Api
{
    ...
    {
        return ['amount' => Decimal::normalize((string)($body['amount'] ?? ''))];
    }
    ...
    {
        $amount = Decimal::normalize((string) ($body['amount'] ?? ''));
        $rate = Decimal::normalize((string) ($body['rate'] ?? ''));
        ...
        if ($timestamp === '' || strtotime($timestamp) === false) {
            throw new InvalidArgumentException('rateTimestamp must be a valid timestamp');
        }
